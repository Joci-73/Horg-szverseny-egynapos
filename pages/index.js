                  ...
                  {competitors.map((competitor, index) => (
                    <React.Fragment key={competitor.id}>
                      ...
                      {competitor.measurements.map((m, mIndex) => (
                        <tr key={m.id} className="bg-gray-50 border-b border-gray-100">
                          <td className="px-2 py-1 text-center text-gray-400 text-xs">{mIndex === 0 ? index + 1 : ''}</td>
                          <td className="px-2 py-1 text-gray-600 text-xs">{mIndex === 0 ? competitor.name : ''}</td>
                          <td className="px-2 py-1 text-center text-green-600">{m.nagyhal > 0 ? m.nagyhal + ' g' : '-'}</td>
                          <td className="px-2 py-1 text-center text-blue-600">{m.aprohal > 0 ? m.aprohal + ' g' : '-'}</td>
                          <td className="px-2 py-1 text-center text-purple-600">{m.darabszam > 0 ? m.darabszam + ' db' : '-'}</td>
                      ...
                      {/* Bevitel sor / Összesítő sor */}
                      <tr className={index % 2 === 0 ? 'bg-white border-b border-gray-200' : 'bg-blue-50 border-b border-gray-200'}>
                        <td className="px-2 py-2 text-center font-bold text-gray-700">{competitor.measurements.length === 0 ? index + 1 : ''}</td>
                        <td className="px-2 py-2 font-semibold text-gray-800">{competitor.measurements.length === 0 ? competitor.name : 'Összesen:'}</td>
